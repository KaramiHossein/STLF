% Special Day
function Final_Forecast=SpecialDay(yy,mm,dd,lsys,Ghcal,calH,ModifiedData)
...
for i=1:length(aa)-1         % I IS THE TRAINING YEAR
    P(:,i)=[(lsys((fft(i)-1)*24-23+length(ModifiedData):(fft(i)-1)*24+length(ModifiedData))-lsys((fft(i)-1)*24-47+length(ModifiedData):(fft(i)-1)*24-24+length(ModifiedData)))./lsys((fft(i)-1)*24-47+length(ModifiedData):(fft(i)-1)*24-24+length(ModifiedData));...
        calcod(i)];
